...
export default function ActionBar({ imageA, imageB, whiteBarHeight }: ActionBarProps) {
  ...
  return (
    <>
      <div className="fixed bottom-0 left-0 right-0 md:hidden bg-black/50 backdrop-blur border-t border-white/10 h-12 flex items-center gap-2 px-2">
        <Button variant="outline" size="sm" className="flex-1 text-xs h-10 bg-transparent" onClick={handleRefresh}>
          <RefreshCw className="w-4 h-4" />
        </Button>
        <Button
          className="flex-1 bg-[#FFE45C] text-[#1C1542] hover:bg-[#FFE45C]/90 text-xs h-10 font-bold"
          onClick={handleCompose}
          disabled={isComposing || !imageA || !imageB}
        >
          {isComposing ? "装填中..." : "上线整活"}
        </Button>
        <Button variant="outline" size="sm" className="flex-1 text-xs h-10 bg-transparent" onClick={handleExport}>
